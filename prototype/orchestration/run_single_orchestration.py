...
# Ensure prototype root is on sys.path for relative imports
...
# Import hybrid processing functions with relative imports
from services.util_services import process_pdf_hybrid
from services.ocr_processing import GoogleVisionOCR
from template_matching.regex_classifier import create_classifier
from services.kag_writer import generate_kag_input, validate_kag_input_file
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AI Backend Prototype - PDF to KAG Pipeline")
    parser.add_argument("--pdf", type=str, 
                       default="prototype/data/test-files/testing-ocr-pdf-1.pdf",
                       help="Path to PDF file to process")
    parser.add_argument("--out", type=str,
                       default="prototype/artifacts/run-001", 
                       help="Output directory for artifacts")
    
    args = parser.parse_args()
    
    # Convert to absolute paths from repo root if relative
    pdf_path = Path(args.pdf)
    output_dir = Path(args.out)
    
    if not pdf_path.is_absolute():
        # Resolve relative to repo root (two levels up from prototype/orchestration/)
    ...
    print(f"🚀 Starting AI Backend Prototype Pipeline")
