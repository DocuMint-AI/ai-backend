...
class DocumentParser:
    ...
    def parse_document(
        ...
        try:
            ...
            needs_review = self._check_needs_review(named_entities, key_value_pairs, clauses)
            
            if needs_review:
                logger.info("Running fallback extraction due to insufficient DocAI results")
                fallback_entities, fallback_kvs, fallback_clauses = self._run_fallback_extraction_for_schema(full_text)
                
                # Merge fallback results
                named_entities.extend(fallback_entities)
                key_value_pairs.extend(fallback_kvs)
    ...
    def _check_needs_review(self, entities: List[NamedEntity], kvs: List[KeyValuePair], clauses: List[Clause]) -> bool:
        """
        Check if document needs manual review due to insufficient extraction.
        ...
        """
        # Check for minimum mandatory fields
        mandatory_kv_types = ["policy_no", "date_of_commencement", "sum_assured", "dob"]
        
        found_mandatory = 0
        for kv in kvs:
            if any(mandatory in kv.key.lower() for mandatory in mandatory_kv_types):
                found_mandatory += 1
